          ...
          <div className="rounded-xl bg-neutral-100 dark:bg-neutral-900 p-3 space-y-2 text-xs">
            <div>
              ...
              <div className="flex items-center gap-1.5">
                <span
                  className={`inline-block w-1.5 h-1.5 rounded-full ${methodPresentation?.dotClassName ?? 'bg-amber-500'}`}
                />
                <span className="font-semibold">
                  {methodPresentation?.label ?? paymentKindLabel(details.paymentKind ?? 'lightning')}
                </span>
                <span className="text-neutral-500 dark:text-neutral-400">
                  — {methodPresentation?.hint ?? 'BOLT11 invoice'}
                </span>
              </div>
            </div>
            <div>
